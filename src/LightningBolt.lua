--Powerful Procedural Lightning Module. By Quasiduck
--License: See GitHub
...
function LightningBolt.new(Attachment0, Attachment1, PartCount)
	...
	--Main (default) Properties--
		...
		--Bolt Appearance Properties--
			...
			self.Frequency = 1 --PartCount/40 --Governs the frequency of fluctuations throughout the bolt
		...
		--Bolt Kinetic Properties--
			...
			self.PulseSpeed = 2 --See https://www.desmos.com/calculator/hg5h4fpfim
		...
		if ThisBranch.Enabled == true then
			...
			if timePassed < (PulseLength + 1) / PulseSpeed then
				...
				for i = 1, PartsN do
					...
					local noise1 = NoiseBetween(5*input2, 5*input, 3.7, 0, 0.1) + NoiseBetween(0.5*input2, 0.5*input, 1.4, 0, 0.9)
					--local noise0 = NoiseBetween(input, 1.5, 0.2*input2, 0, 2*math.pi)
					--local noise1 = NoiseBetween(input2, input, 2.7, 0, 1)
					local thicknessNoise = NoiseBetween(2.3, input2, input, MinThick, MaxThick)
					local NextPoint = i ~= PartsN and (CFrame.lookAt(bezier0, bezier1)*CFrame.Angles(0, 0, noise0)*CFrame.new(math.exp(-5*(t1 - 0.5)^2)*(MinRadius + (MaxRadius - MinRadius)*math.sqrt(math.max(0, noise1))), 0, -1)).Position or bezier1
